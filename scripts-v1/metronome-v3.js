...
loadSound('https://zero-network.net/the-other-side/static/assets/footstep_1.mp3',0)
...
function draw_graph(clear = true){
    ...
    c.beginPath();
    c.moveTo(15,5)
    c.lineTo(15,c.canvas.height - 15)
    c.lineTo(c.canvas.width,c.canvas.height - 15)
    c.stroke();
    ...
    for (var i = 0; i <= hlen-1; i+=10){
        c.fillText(i, (c.canvas.width-15) / hlen * i + 15, c.canvas.height - 5)
        c.beginPath();
        c.setLineDash([2,1])
        c.moveTo((c.canvas.width-15) / hlen * i + 15,c.canvas.height - 5)
        c.lineTo((c.canvas.width-15) / hlen * i + 15,5)
        c.stroke();
    }
    ...
    for (var i = 0; i <= 4.0; i+=0.5){
        c.fillText(i, 10, (c.canvas.height-20) - ((c.canvas.height-20) / 4 * i) + 5)
        c.beginPath();
        c.setLineDash([2,1])
        c.moveTo(15,(c.canvas.height-20) - ((c.canvas.height-20) / 4 * i) + 5)
        c.lineTo(c.canvas.width,(c.canvas.height-20) - ((c.canvas.height-20) / 4 * i) + 5)
        c.stroke();
    }
    ...
    if(bpm_hist.length > 0){
        ...
        if(clear)
            ...
            c.setLineDash([1,0])
        c.moveTo(15,(c.canvas.height-20) - ((c.canvas.height-20) / 4 * bpm_hist[0]) + 5)
        for(var i = 1; i < bpm_hist.length; i++){
            var cur_x = (c.canvas.width-15) / hlen * bpm_hist[i]['seconds'] + 15
            var cur_y = (c.canvas.height-20) - ((c.canvas.height-20) / 4 * bpm_hist[i]['speed']) + 5
...
function graph_bpm(){
    ...
    var cur_x = (c.canvas.width-15) / hlen * (bpm_hist[bpm_hist.length - 1]['seconds']) + 15
    var cur_y = (c.canvas.height-20) - ((c.canvas.height-20) / 4 * bpm_hist[bpm_hist.length - 1]['speed']) + 5
    if(bpm_hist.length > 1){
        var prev_x = (c.canvas.width-15) / hlen * (bpm_hist[bpm_hist.length - 2]['seconds']) + 15
        var prev_y = (c.canvas.height-20) - ((c.canvas.height-20) / 4 * bpm_hist[bpm_hist.length - 2]['speed']) + 5
    }
    else{
        var prev_x = cur_x
        var prev_y = cur_y
    ...
    for (var i = 0; i < ghosts.length; i++){
        ...
        if(ms != 0.00){
            ...
            if(document.getElementById("bpm_type").checked){
                ...
                else if (speed != los_speed && (
                        (speed > los_speed && los_speed <= ms && ms < speed) ||
                        (speed < los_speed && speed < ms && ms <= los_speed)
                    )
                ){
...
function calibrateOffset(speed){
    if (calibrating){
        ...
        else if (speed <= 2.5 && speed > 2.4){
            offset += 0.5
            document.getElementById("cal_complete").innerText = lang_data['{{calibrating}}']
            num_correct = 0
        }
        else if (speed <= 2.4 && speed > 2.36){
            offset += 0.1
            document.getElementById("cal_complete").innerText = lang_data['{{calibrating}}']
            num_correct = 0
        }
        else if(speed <= 2.36 && speed >= 2.34){
            num_correct += 1
            if (num_correct == 5){
        ...
        else if (speed < 2.34 && speed >= 2.30){
            offset -= 0.1
            document.getElementById("cal_complete").innerText = lang_data['{{calibrating}}']
            num_correct = 0
        }
        else if (speed < 2.30 && speed >= 2.25){
            offset -= 0.5
            document.getElementById("cal_complete").innerText = lang_data['{{calibrating}}']
            num_correct = 0
        }
